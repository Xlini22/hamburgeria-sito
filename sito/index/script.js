...
function updateCarouselControls() {
  ...
  nextProducts.setAttribute('aria-label', isLastPage
...
productTrack.addEventListener('touchmove', event => {
  if (!window.matchMedia('(max-width: 720px)').matches) return;
  const touch = event.touches[0];
  ...
  const isBeyondLast = currentProductPage === productPages.length - 1 && distanceX < 0;
  if (isBeyondFirst || isBeyondLast) dragDistance *= .28;
...
function finishProductDrag() {
  ...
  const shouldChangePage = Math.abs(distanceX) > pageWidth * .18 || velocity > .45;
  ...
  isDraggingProducts = false;
  window.setTimeout(() => { suppressProductClick = false; }, 350);
}
